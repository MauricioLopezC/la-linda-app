...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('supplier_vouchers_hu026', function (Blueprint $table) {
            $table->id();
            $table->foreignId('supplier_id')->constrained()->restrictOnDelete();
            $table->foreignId('warehouse_id')->nullable()->constrained('warehouses')->restrictOnDelete();
            $table->rawColumn('type', "varchar(20) check (type in ('factura', 'nota_credito', 'nota_debito', 'remito'))");
            $table->rawColumn('letter', "varchar(1) check (letter in ('A', 'B', 'C', 'M', 'R', 'X'))")->default('A');
            ...
            $table->rawColumn(
                'status',
                "varchar(30) check (status in ('pendiente', 'pagada_parcial', 'pagada', 'pendiente_imputar', 'imputada_parcial', 'imputada', 'anulada', 'confirmado'))"
            );
            ...
            $table->unique(
                ...
                'supplier_vouchers_hu026_fiscal_unique'
            );
        ...
        DB::table('supplier_vouchers_hu026')->insertUsing(
            [
                'id', 'supplier_id', 'warehouse_id', 'type', 'letter', 'point_of_sale', 'number',
            ...
            DB::table('supplier_vouchers')->select([
                'id', 'supplier_id', DB::raw('NULL'), 'type', 'letter', 'point_of_sale', 'number',
                'issue_date', 'due_date', 'total_amount', 'status', 'notes',
                'annulled_at', 'annulled_by', 'annulment_reason', 'created_at', 'updated_at',
            ])
        );
        ...
        Schema::rename('supplier_vouchers_hu026', 'supplier_vouchers');
    ...
    {
        if (DB::table('supplier_vouchers')
            ->where('type', 'remito')
            ->orWhere('total_amount', '<=', 0)
            ->orWhereIn('letter', ['R', 'X'])
            ->orWhere('status', 'confirmado')
            ->exists()) {
            throw new RuntimeException(
                'No se puede revertir la migración HU-026: existen comprobantes incompatibles con el esquema anterior.'
            );
        }
