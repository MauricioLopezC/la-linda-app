...
Done, closing HU-017: `sign` is nullable (migration `2026_08_23_154747_make_sign_nullable_on_stock_movement_types_table`), meaning "varies per line", and seeded `null` for `inventory_adjustment` and `warehouse_transfer` (the other three keep a fixed sign). Before this, the seeder stored `1` for both, which was false — an adjustment varies per line and a transfer is two rows with opposite effects — and the ABM badge showed that false value to the user. Store/Update requests validate `sign` as `nullable`+`Rule::in([1,-1])`; `StockMovementType::$sign` and `StockMovementTypeData::$sign` are `int|null`; the ABM badge (`parameters/index.tsx`) shows a third "Varía por renglón" state for null, read straight from `sign` (never derived from `code`).

Pending, for whoever takes HU-019/HU-026: in the movement action, assert the line's sign matches the type's when the type defines one; accept any sign when it is null. Nothing in the schema stops a `sale_exit` line with a positive quantity — `CHECK (quantity <> 0)` only rules out zero, and a wrong sign is permanent because movements are immutable. For `warehouse_transfer` specifically, the type's null sign gives no protection by itself — the real invariant to enforce is that the two lines' deltas sum to zero.
