...
type MovementTypeOption = App.Data.Inventory.StockMovementTypeData;
...
export default function CreateStockAdjustment({
  ...
  const { data, setData, post, processing, errors, transform } = useForm({
    ...
    notes: '',
    items: [] as Array<{ article_id: number; quantity: number }>,
  });

  const [itemsDraft, setItemsDraft] = useState<MovementItemDraft[]>([]);
  ...
  const selectedType = movementTypes.find(
    (t) => String(t.id) === data.stock_movement_type_id,
  );
  const isDeduction = selectedType?.sign === -1;
  const canAddArticles = Boolean(
    data.warehouse_id && data.stock_movement_type_id,
  );
  ...
  }, [searchTerm, data.warehouse_id, canAddArticles]);
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Elegí el tipo de movimiento y cargá únicamente la cantidad que
              entra o sale de cada artículo. El sistema calcula el nuevo saldo.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Card>
            <CardHeader className="pb-4">
              ...
              <CardDescription>
                Indicá el depósito, el tipo de movimiento y la justificación
                obligatoria.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="stock_movement_type_id">
                  Tipo de movimiento <span className="text-destructive">*</span>
                </Label>
                <Select
                  value={data.stock_movement_type_id}
                  onValueChange={(val) =>
                    setData('stock_movement_type_id', val)
                  }
                >
                  <SelectTrigger id="stock_movement_type_id" className="w-full">
                    <SelectValue placeholder="Seleccioná el tipo..." />
                  </SelectTrigger>
                  <SelectContent>
                    {movementTypes.map((t) => (
                      <SelectItem key={t.id} value={String(t.id)}>
                        {t.name} {t.sign === 1 ? '(entra +)' : '(sale −)'}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                {selectedType && (
                  ...
                  >
                    ...
                    {isDeduction
                      ? 'Este movimiento resta existencias'
                      : 'Este movimiento suma existencias'}
                  </Badge>
                )}
                {errors.stock_movement_type_id && (
                  <p className="text-xs text-destructive">
                    {errors.stock_movement_type_id}
                  </p>
                )}
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Detalle o circunstancia del movimiento (ej: Mercadería rota detectada durante descarga de camión)..."
                  rows={2}
                  maxLength={500}
              ...
              <div className="relative">
                <div className="relative flex items-center">
                  ...
                  <Input
                    ...
                    placeholder={
                      !canAddArticles
                        ? 'Elegí depósito y tipo de movimiento arriba para habilitar la búsqueda...'
                        : 'Escribí nombre, código interno (ART-...) o escaneá código de barras...'
                    }
                    disabled={!canAddArticles}
              ...
              {itemsDraft.length === 0 ? (
                <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed p-8 text-center text-muted-foreground">
                  ...
                  <p className="max-w-sm text-xs">
                    Utilizá el buscador superior para agregar los productos de
                    este movimiento.
                  </p>
                </div>
              ) : (
        ...
        <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>¿Confirmar registro del movimiento?</DialogTitle>
              <DialogDescription asChild>
                <div className="space-y-3 pt-2 text-sm text-muted-foreground">
                  <p>Estás a punto de confirmar el siguiente movimiento:</p>
